...
function Card() {
  const [src, setSrc] = useState('')
    ...
    const generateDataForDropdown = () => {
      return [...new Set(List.map((List) => List.category))];
    };
    const handleFilter = (g) => {
      const filteredData = List.filter((List) => {
     ...
     useEffect(() => {
      ...
      axios.get("http://localhost:3005/login").then((response) => {
        if (response.data.loggedIn == true){
          setLoginStatus(response.data.user.id).then(getList())
          // getList(); 
        }
       else{
      navigate(`/login`)
...
{List.map((List) => (
  ...
  <div className='det'>
    ...
    <Button  color="link" onClick={()=> navigate(`/details/${List.id}`)} style={{color:'#303f9f'}} >View More</Button>
    </div>
    </div>
  </div>
  </div>
